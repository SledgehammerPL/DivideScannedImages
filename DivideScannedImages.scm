...
(define (script_fu_DivideScannedImages img inLayer inThreshold inSize inLimit inCorner inX inY inSaveFiles inDir inSaveType inFileName inFileNumber)
  ...
  (cond 
    ( (equal? inCorner 0)
      (gimp-fuzzy-select inLayer (+ inX 16) (+ inY 16) inThreshold CHANNEL-OP-REPLACE TRUE FALSE 0 TRUE)
  ...
  (gimp-selection-grow img 50) ; unshrink for boundaries
  (gimp-selection-invert img)
  ...
  (while (and (< count (car strokes)) (< numextracted inLimit))
    ...
    (if (and (> (- (list-ref bounds 3) (list-ref bounds 1)) inSize) (> (- (list-ref bounds 4) (list-ref bounds 2)) inSize) ;min size slider
      ...
      (begin
        (gimp-rect-select img (list-ref bounds 1) (list-ref bounds 2) (- (list-ref bounds 3) (list-ref bounds 1)) (- (list-ref bounds 4) (list-ref bounds 2)) CHANNEL-OP-REPLACE FALSE 0 )
        (set! buffname0 (car (gimp-edit-named-copy inLayer buffname)))
        (set! tempImage0 (car (gimp-edit-named-paste-as-new buffname0)))
        (set! tempLayer0 (car (gimp-image-get-active-layer tempImage0)))
        (gimp-image-undo-disable tempImage0)
        (set! tempdisplay0 (car (gimp-display-new tempImage0)))

        ;run deskew if it is installed
        (if (defined? 'gimp-deskew-plugin) 
          (begin
	    (gimp-progress-set-text "Deskewing...")
	    (gimp-layer-flatten tempLayer0)
	    (gimp-deskew-plugin 0 tempImage0 tempLayer0 0 0 0 0 0)
	    (gimp-image-resize-to-layers tempImage0)
	    (gimp-layer-flatten tempLayer0)
	    (gimp-fuzzy-select tempLayer0 0 0 10 CHANNEL-OP-REPLACE TRUE FALSE 0 TRUE) ; threshold>10
	    (gimp-selection-invert tempImage0)
	    (gimp-selection-shrink tempImage0 10)
	    (gimp-selection-grow tempImage0 10) ; unshrink for boundaries

	    (gimp-selection-invert tempImage0)

	    (plug-in-sel2path RUN-NONINTERACTIVE tempImage0 tempLayer0)

	    ;break up the vectors
	    (set! newpath1 (vector-ref (cadr (gimp-image-get-vectors tempImage0)) 0)) 

	    (set! strokes1 (gimp-vectors-get-strokes newpath1))
            (define count1 0)
	    (while (< count1 (car strokes1))

              (set! tempVector1 (gimp-vectors-new tempImage0 "Temp"))
	      (gimp-image-add-vectors tempImage0 (car tempVector1) -1)
	      (gimp-vectors-stroke-new-from-points (car tempVector1)
	        (list-ref (gimp-vectors-stroke-get-points newpath1 (vector-ref (cadr strokes1) count1)) 0)
	        (list-ref (gimp-vectors-stroke-get-points newpath1 (vector-ref (cadr strokes1) count1)) 1)
	        (list-ref (gimp-vectors-stroke-get-points newpath1 (vector-ref (cadr strokes1) count1)) 2)
	        (list-ref (gimp-vectors-stroke-get-points newpath1 (vector-ref (cadr strokes1) count1)) 3)
							       )
	        (gimp-vectors-to-selection (car tempVector1) CHANNEL-OP-REPLACE TRUE FALSE 0 0)

	        ;check for minimum size
	        (set! bounds1 (gimp-selection-bounds tempImage0))
	        (if (= count1 1)
	          (begin
;	            (gimp-rect-select tempImage0 (list-ref bounds1 1) (list-ref bounds1 2) (- (list-ref bounds1 3) (list-ref bounds1 1)) (- (list-ref bounds1 4) (list-ref bounds1 2)) CHANNEL-OP-REPLACE FALSE 0 )

                    (set! buffname (car (gimp-edit-named-copy tempLayer0 buffname)))
		    ...
		    (gimp-image-undo-enable tempImage)
                ...
                (gimp-image-remove-vectors tempImage0 (car tempVector1))
                (set! count1 (+ count1 1))
	      )
	      (gimp-display-delete tempdisplay0)
	    )
	  )
...
(script-fu-register "script_fu_DivideScannedImages"
		    ...
		    SF-ADJUSTMENT "Abort Limit"                         (list 15 1 100 1 10 1 SF-SLIDER)                         
